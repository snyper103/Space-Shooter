...
public class player2 : MonoBehaviour
{
    ...
    {
        if ( !_laserPrefab )
            Debug.LogError("player2::_laserPrefab is NULL");

        if ( !_tripleFirePrefab )
            Debug.LogError("player2::_tripleFirePrefab is NULL");

        if ( !_shieldPrefab )
            Debug.LogError("player2::_shieldPrefab is NULL");
        
        if ( !_spawnManager )
            Debug.LogError("player2::_spawnManager is NULL");

        if ( !_explosionPrefab )
            Debug.LogError("player2::_explosionPrefab is NULL");

        if ( !_UImg )
            Debug.LogError("player2::_UImg is NULL");

        if ( !_damage[0] )
            Debug.LogError("player2::_damage[0] is NULL");

        if ( !_damage[1] )
            Debug.LogError("player2::_damage[1] is NULL");

        if ( !_thrusterPrefab )
            Debug.LogError("player2::_thrusterPrefab is NULL");

        if ( !_movementAnim )
            Debug.LogError("player2::_movementAnim is NULL");
    }
    ...
    {
        _movement.x = Input.GetAxis("Horizontal2");
        Vector2 speedX = new Vector2(_movement.x, 0);
        _movement.y = Input.GetAxis("Vertical2");
    ...
    void shootingLaser()
    {
        ...
        if ( _tripleShotEnable )
        {
            ...
            for ( int i = 0; i < 3; ++i )
                newLaser.transform.GetChild(i).gameObject.GetComponent<laser>().playerIndex(1);
        }
        ...
        else
        {
            ...
            newLaser.GetComponent<laser>().playerIndex(1);
        }
    }
        ...
        switch ( powerUpID )
        {
            ...
            case 2:
                ...
                if ( !_shield.transform.parent )
                    Debug.LogError("player2::_shield.transform.parent is NULL");
                break;
            default:
                break;
        }
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {   
            ...
            if ( !myEnemy )
                Debug.LogError("player2::myEnemy is NULL");
            ...
            if ( _hp > 0 )
            {
                if ( !_shieldEnable )
                {
                    ...
                    switch ( _hp )
                    {
                        ...
                        default:
                            ...
                            _spawnManager.onPlayersDeath(1);
                            _speed = 0.0f;
                ...
                myEnemy.activeDestruction(false, 1);
        ...
        else
        {
            if ( other.tag == "PowerUp" )
            {
                ...
                if ( !myPower )
                    Debug.LogError("player2::myPower is NULL");
